<template>
  <div class="main-content">
      ...
      <el-form :inline="true" :model="searchForm" class="form-content">
        <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                  ...
                  <el-input v-if="contents.inputIcon == 0" v-model="searchForm.fangwubianhao" placeholder="房屋编号" clearable></el-input>
                                                                              ...
                                                                              <el-form-item :label="contents.inputTitle == 1 ? '房源类型' : ''">
                  <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 1" prefix-icon="el-icon-search" v-model="searchForm.fangyuanleixing" placeholder="房源类型" clearable></el-input>
                  <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 2" suffix-icon="el-icon-search" v-model="searchForm.fangyuanleixing" placeholder="房源类型" clearable></el-input>
                  <el-input v-if="contents.inputIcon == 0" v-model="searchForm.fangyuanleixing" placeholder="房源类型" clearable></el-input>
                </el-form-item>
		    ...
		    label="详细地址">
		     <template slot-scope="scope">
                ...
                <el-button v-if="isAuth('fangwuxinxi','查看') && contents.tableBtnIcon == 0" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                                                                <el-button v-if="isAuth('fangwuxinxi','看房申请') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="kanfangshenqingCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'看房申请':'' }}</el-button>
                <el-button v-if="isAuth('fangwuxinxi','看房申请') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="kanfangshenqingCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'看房申请':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('fangwuxinxi','看房申请') && contents.tableBtnIcon == 0" type="success" size="mini" @click="kanfangshenqingCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'看房申请':'' }}</el-button>
    ...
    <add-or-update v-if="addOrUpdateFlag" :parent="this" ref="addOrUpdate"></add-or-update>
            ...
            <kanfangshenqing-cross-add-or-update v-if="kanfangshenqingCrossAddOrUpdateFlag" :parent="this" ref="kanfangshenqingCrossaddOrUpdate"></kanfangshenqing-cross-add-or-update>
...
import kanfangshenqingCrossAddOrUpdate from "../kanfangshenqing/add-or-update";
export default {
  data() {
    return {
                                                                                                                                                                                                                                                                                                                                                                  searchForm: {
      ...
      addOrUpdateFlag:false,
                  kanfangshenqingCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
            kanfangshenqingCrossAddOrUpdate,
          },
  methods: {
    contentStyleChange() {
    ...
    contentPageStyleChange(){
      ...
      this.contents.pageEachNum = 10
            ...
            kanfangshenqingCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.kanfangshenqingCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','fangwuxinxi');
      this.$nextTick(() => {
        this.$refs.kanfangshenqingCrossaddOrUpdate.init(row.id,type);
      });
    },
                    init () {
                                                                                                          this.zhuangtaiOptions = "已租,待租".split(',')
                                                                                                                                                                                                                                                                                                                                                                                                                            },
    search() {
      this.pageIndex = 1;
      this.getDataList();
