<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="户主姓名" prop="huzhuxingming">
          <el-input v-model="ruleForm.huzhuxingming" 
              placeholder="户主姓名" clearable  :readonly="ro.huzhuxingming"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="户主姓名" prop="huzhuxingming">
              <el-input v-model="ruleForm.huzhuxingming" 
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `huzhu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.huzhuCrossAddOrUpdateFlag = false;
                  this.parent.search();
